			...
			public void run() {
				if (globalVariable.isInternet() == true
					...
					try {
						...
						String uuid = imeiNo + UUID.randomUUID().toString()
								+ sdf.format(new Date());
						...
						JSONObject jsonResult = new JSONParser()
								.getJSONFromUrlAfterHttpPost(url,
										new JSONObject().put("counter",
												jsonObject));
						System.out.println(">>>>>>>>> inside counter post");
						if (jsonResult.getString("status").equals("ok")) {
							...
							globalVariable.setTotalInDB(jsonResult
									.getInt("total"));
						}
